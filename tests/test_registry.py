...
class TestRegistry:
    ...
    def test_register_decorator(self):
        ...
        assert self.registry.find(name="test_12")[0] is test_registry_decorator
    ...
    def test_find_from_fallback(self):
        ...
        assert self.registry.find(name="fallback")[0] == "fallback_obj"

    def test_remove_object(self):
        # Test removing with just the object
        self.registry.remove("obj_1")
        with pytest.raises(KeyError):
            assert self.registry.find(name="test_1")

        with pytest.raises(KeyError):
            # assert that every single reference is removed
            assert self.registry.find(name2="test_1")

    def test_remove_keyword(self):
        # Test removing with just kwargs:
        self.registry.remove(name="test_2")
        with pytest.raises(KeyError):
            assert self.registry.find(name="test_2")

        with pytest.raises(KeyError):
            # assert that every single reference is removed
            assert self.registry.find(name2="test_2")

        self.registry.remove(name="test_3", name2="test_3")
        with pytest.raises(KeyError):
            assert self.registry.find(name2="test_3")
